...
class TransformVisitor:
    ...
    def __call__(self, bsym: BoundSymbol) -> VISIT_TYPE:
        ...
        if bsym in self.bsyms_before_allgather:
            # To be more technically precise, this should check the proxy != params.
            orig_input: TensorProxy = bsym.flat_proxy_args[0]
            utils.check_type(orig_input, TensorProxy)
            input_scattered_for_row_wise_op = dist_prims.synchronize_input_for_row_wise_tensor_parallel(
                orig_input,
                self.process_group,
            )
            self.swap_map[variableify(orig_input)] = input_scattered_for_row_wise_op
        ...
        if bsym in self.bsyms_before_allgather:
            ...
            gathered_output = dist_prims.synchronize_output_for_row_wise_tensor_parallel(
                output_to_gather,
                self.process_group,
            )
...
@dataclass
class TransformForRowWiseParallel:
    rank: int
    world_size: int
...
# TODO(crcrpar): Add an option to turn off input reduce-scatter and output all-gather.
def convert_module_to_rowwise_parallel(
    ...
    *,
    ...
    """Convert specified moduls into row-wise parallel ones."""
    ...
    for target_mod_name in target_modules:
        ...
        for name, p in mod.named_parameters(recurse=False):
            _shard_param(p, rank, world_size, name, dim=1)
